import { TareaTipo } from "./contructora";
...
export function mostrarTareas(tareas:TareaTipo[]):void{ //tareas es de tipo Tarea[] osea es un array que contiene objetos de tipo Tarea
...
export function mostrarPendientes(tareas:TareaTipo[]):void{
  let filtroPendiente: TareaTipo[]=[];//filtro pendiente es de tipo arraytarea y esun array
...
export function mostrarCurso(tareas:TareaTipo[]):void{
  let filtroCurso:TareaTipo[]=[];
...
export function mostrarTerminadas(tareas:TareaTipo[]):void{
  let filtroTerminadas:TareaTipo[]=[];
...
export function ordenarFechas(tareas: TareaTipo[]):void{
...
export function mostrarDetalles(tareas:TareaTipo[]):void{
  let opcion:number;
  do{
    ...
    let t:TareaTipo=tareas[opcion-1];//acceso directo a tarea
